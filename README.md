...
### What it does
Using the [Deezer Python Client](https://github.com/browniebroke/deezer-python), it takes my favorite artists and fetches their latest releases ("_latest_" means since the last 7 days by default but `NB_DAYS_DELTA` can be changed in `main.py`). Then, it puts them in a page in my Notion.so account, in which I have a ":fire: New releases" section. This code is automatically executed each week. Hopefully, I'll be kept up to date!
